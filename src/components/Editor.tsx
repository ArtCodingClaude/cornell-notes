...
export function Editor({ note, focusRequest, onBack }: Props) {
  ...
  function move(from: Section, direction: 1 | -1) {
    const next = order[(order.indexOf(from) + direction + order.length) % order.length]
...
function SectionBox({
  ...
  return (
    <section
      className={cx(
        'flex min-w-0 flex-col rounded-2xl border-2 bg-[var(--surface)] transition-[border-color,box-shadow] duration-200',
      )}
      style={{
        ...style,
